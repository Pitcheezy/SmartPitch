...
### Task 1 — 범용 MLP 전환 (전체 MLB 데이터로 재학습)

**왜 필요한가**
현재 MLP는 단일 투수 데이터(~2,000~3,000건)로 학습되어 val_acc 47% 수준이다.
MLP는 MDP와 DQN 시뮬레이터의 핵심이므로 여기서 틀리면 모든 결과가 신뢰를 잃는다.

**평가**
- [병목] ✅ MDP, DQN 모두 이 MLP에 의존
- [임팩트] ✅ val_acc 47% → 65%+ 목표, 전체 파이프라인 정확도 직결
- [신뢰도] ✅ 현재 결과는 단일 투수 편향 학습이라 MLB 전반에 적용 불가

**수정 파일**
- `src/main.py` — TransitionProbabilityModel에 전체 MLB 데이터를 전달하는 로직 추가
- `src/model.py` — universal=True 모드 또는 별도 진입점 추가 (데이터 로드 방식 변경)

**구현 방향**
```python
# batter_clustering.py의 raw_df(72만 건)를 재활용
# model.py에 전체 데이터 학습 모드 추가
# epochs: 5 → 20, batch_size: 64 → 1024
```

**예상 난이도**: 어려움
**선행 작업**: 없음 (data/batter_clusters_2023.csv, pitcher_clusters_2023.csv 이미 존재)
